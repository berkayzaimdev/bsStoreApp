...
namespace Services
{
    public class AuthenticationManager : IAuthenticationService
    {
        ...
        {
            var signingCredientals = GetSigningCredentials();
            var claims = await GetClaims();
            var tokenOptions = GenerateTokenOptions(signingCredientals, claims);
        ...
        private JwtSecurityToken GenerateTokenOptions(object signingCredentials, List<Claim> claims)
        {
            var jwtSettings = _configuration.GetSection("JwtSettings");

            var tokenOptions = new JwtSecurityToken(
                issuer: jwtSettings["validIsssuer"],
